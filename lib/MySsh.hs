...
sshPrompt c = do
  sc <- io sshComplList
  mkXPrompt Ssh c (complFunc sc) ssh
    where complFunc :: [String] -> String -> IO [String]
          complFunc _ [] = return []
          complFunc l s =
            return $ filter (searchPredicate c s) l

ssh :: String -> X ()
ssh host = unsafeSpawn $ "gnome-terminal -e '/usr/bin/ssh -t " ++ host ++ " \"zsh -l\"'"
